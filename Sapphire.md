...
大体のコマンドは、以下のオプションを持っています。

| オプション名 | 説明                                             | 💡Hint                                             | 必須か |
| :----------- | :----------------------------------------------- | :------------------------------------------------- | :----: |
| `user`       | 対象のユーザーを指定します。                     | ユーザー名や ID からユーザーを選択できます。       |   ✅   |
| `duration`   | アクションの有効期間を指定します。               | 例：`1h`（1 時間）                                 |   ❌   |
| `reason`     | アクションの理由を入力します。                   | 理由は必ず入力してください。                       |   ✅   |
| `attachment` | アクションの証拠を提供するためのオプションです。 | このオプションには画像ファイルなどを指定できます。 |   ❌   |
| `review`     | 現在は使用していないため、無視してください。     | 現在は使用していないため、無視してください。       |   ❌   |
...
> [!TIP]
>
> `attachment`オプションは、アクションの証拠を提供するためのものです。画像ファイルなどを指定できます。
>
> 例えば、スクリーンショットやログファイルなどを添付することができます。

> [!IMPORTANT]
>
> `reason`オプションは、必ず入力してください。入力がない場合、無効になる場合があります。
>
> また、`reason`は、ユーザーに通知されるため、適切な理由を入力してください。
...
これは、`purge until here` コマンドのデモ動画です。YouTube ショートでご覧いただけます。
